front.controller("showServiceController", ['$scope', '$http', '$window', '$location', '$filter', '$sce', function ($scope, $http, $window, $location, $filter, $sce) {
    //VAR
    $scope.post = {};
    $scope.categoryPost = {};
    $scope.relatePosts = [];
    ...
    function GetPost() {
        $http.get('/Service/GetPost/' + $scope.idPost)
            .then(
                function success(response) {
                    if (response.status == 200) {
                        ...
                        $scope.relatePosts = angular.copy(response.data.LastPosts);
